...
drop policy if exists "Students can create their own viewing requests" on public.viewings;
drop policy if exists "Viewing participants can update viewing status" on public.viewings;
drop policy if exists "Viewing owners can update their viewing requests" on public.viewings;
drop policy if exists "Viewing requesters can cancel their own viewing requests" on public.viewings;

create policy "Students can request custom viewings after accepted inquiry"
on public.viewings
for insert
to authenticated
with check (
  ...
  and exists (
    ...
    where i.id = inquiry_id
      and i.listing_id = viewings.listing_id
      and i.owner_id = viewings.owner_id
      and i.requester_id = auth.uid()
      and i.status = 'accepted'
      and l.user_id = viewings.owner_id
      and l.status = 'available'
      and l.user_id <> auth.uid()
  )
);

create policy "Viewing owners can update their viewing requests"
on public.viewings
for update
to authenticated
...
create policy "Viewing requesters can cancel their own viewing requests"
on public.viewings
for update
to authenticated
using (auth.uid() = requester_id and status in ('pending', 'suggested'))
with check (auth.uid() = requester_id and status = 'declined');
