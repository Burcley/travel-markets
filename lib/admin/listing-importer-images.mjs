...
export function assignImageNamesToRow(currentAssignments = {}, fingerprint, imageNames = []) {
  if (!fingerprint) return currentAssignments;

  const existing = currentAssignments[fingerprint] || [];
  const additions = imageNames.filter((imageName) => imageName && !existing.includes(imageName));

  return {
    ...currentAssignments,
    [fingerprint]: [...existing, ...additions],
  };
}
