...
export const LISTING_DRAFT_INSERT_COLUMNS = [
  "user_id",
  "title",
  "city",
  "location",
  ...
  "public_location_generated_at",
  "nearest_campus_name",
  "nearest_campus_address",
  "campus_id",
  "campus_destination_label",
  "campus_coordinate_source",
  "campus_latitude",
  "campus_longitude",
  "distance_to_campus_km",
  "walking_time_minutes",
  "cycling_time_minutes",
  "driving_time_minutes",
  "transit_time_minutes",
  "distance_last_calculated_at",
  "utilities_details",
  "amenities_details",
  "lease_conditions",
...
export function schemaCompatibleListingInsertPayload(payload) {
  const allowedColumns = new Set(LISTING_DRAFT_INSERT_COLUMNS);
...
export function collectAssignedImportImages({
  rows = [],
  imageAssignments = {},
  imageFiles = [],
} = {}) {
  ...
  rows.forEach((row) => {
    ...
    (imageAssignments[fingerprint] || []).forEach((imageName, index) => {
      if (!availableNames.has(imageName)) return;

      const key = `${fingerprint}:${imageName}`;
      if (seen.has(key)) return;
      seen.add(key);

      const file = imageFiles.find((image) =>
        typeof image === "string" ? image === imageName : image?.name === imageName
      );
      ...
      assignedImages.push({
        ...
        isCover: index === 0,
        contentType:
          file && typeof file !== "string" && typeof file.type === "string"
            ? file.type
            : null,
        size:
...
export function buildDraftListingPayload({ ownerId, row, city = "", province = "Ontario" }) {
  ...
  return schemaCompatibleListingInsertPayload({
    user_id: ownerId,
